...
def dofeedbackcontrol():
    ...
    while not rospy.is_shutdown():
        ...
        if not guided_mode:
            if print_stat_test: print("NOT GUIDED ...")
            above_object = False
            moving_to_set_alt = False
            OPT_FLOW = False # turn off the optical flow mode
            if print_flags: print('above_object = False | moving_to_set_alt = False')
            OPT_COMPUTE_FLAG = False
            if forward_scan_option:
                # turn this initial mode back on
                forward_scan = True
                if print_flags: print('forward_scan = True')
        elif (time_lastbox != None and (rospy.Time.now() - time_lastbox < rospy.Duration(.5)) and not moving_to_set_alt):             
            above_object = True
            if print_flags: print('above_object = True')
            ...
            if forward_scan_option and forward_scan and alt > alt_flow :
            ...
            if fixed_heading_option and not moving_to_set_alt: # only do this if not already moving to setpoint                    
                if print_flags: print("fixed_heading_option = True | moving_to_set_altitude = False")
                # surveying the flow and yawing to head towards the direction of the flow
                fspeed_head, hspeed_head = survey_flow()
            ...
            if gps_alt > 265:
                ...
                fspeed = 0
                alt_set_appr_speed = -1
            if print_stat_test: print(f'fspeed: {fspeed}, traverse_gain: {traverse_gain}, verticalerror: {verticalerror}') 
        ...
        if print_stat_test: print(f'x_speed: {fspeed} | y_speed: {hspeed} | z_speed: {vspeed} | yaw_speed: {yawrate}')
